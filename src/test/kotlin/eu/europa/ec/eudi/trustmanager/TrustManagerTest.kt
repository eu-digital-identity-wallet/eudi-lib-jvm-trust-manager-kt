...
class TrustManagerTest {
    ...
    @Test
    fun testCertificatePathValidation() = runBlocking {
        ...
        val config = TrustManagerConfig.Builder()
            .withCurrentDate(currentDate)
            .withTrustStore(listOf(sampleIACACertificate))
            .withCRLValidationEnabled(false)
            .build()

        val result = TrustManager(config)
            .validateCertificatePath(listOf(dsCertificate))

        assert(result is CertificatePathVerificationResult.Success)
    }

    @Test
    fun testVerifyIssuerAuthAndNamespaces() = runBlocking {
        val sampleIACACertificate = CertificateFactory.getInstance("X.509").generateCertificate(
            ByteArrayInputStream(
                Base64.getDecoder().decode(
                    String(
        ...
        val deviceResponse =
            Hex.decode(ClassLoader.getSystemResourceAsStream("sample_device_response_hex")?.readAllBytes())
